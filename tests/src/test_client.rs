...
use android::aidl::tests::{
    ...
    SimpleParcelable::SimpleParcelable, StructuredParcelable, Union,
};
...
// Enum bitwise-operator + `get()` accessor tests — exercise the impls
// generated by `declare_binder_enum!` (`test_enum_shift` shifts by `get()`,
// which has no generated shift operator). Pure local arithmetic, no binder
// round-trip, so they run on any target without an emulator.
#[test]
fn test_enum_or() {
...
fn test_reverse_simple_parcelables() {
    ...
    let reversed = service
        .ReverseSimpleParcelables(&input, &mut repeated)
        .expect("ReverseSimpleParcelables");
    assert_eq!(repeated, input);
...
#[test]
fn test_vintf_holder_in_local_holder_roundtrip_in_process() {
    // Hermetic (no binder) regression for a generated-codegen bug: a
    // @VintfStability parcelable nested in a LOCAL ParcelableHolder must
    // survive a parcel round trip. The generated `read_from_parcel` reads
    // holder fields via `read_onto` so the holder keeps its pre-set
    // (Vintf) stability; reading it fresh would reset it to Local and reject
    // the wire stability with BadValue. See plans/5-aosp-test-porting.md §5.
    ...
    ep.ext.set_parcelable(Arc::clone(&vintf_ext)).unwrap();

    let mut parcel = Parcel::new();
    ep.write_to_parcel(&mut parcel).expect("write ep");
    parcel.set_data_position(0);
    let mut ep2 = ExtendableParcelable::default();
    ep2.read_from_parcel(&mut parcel).expect("read ep");
    assert_eq!(ep2.a, 1);
    assert_eq!(ep2.b, "a");
    let ret_ext = ep2
        .ext
        .get_parcelable::<VintfExtendableParcelable>()
        .expect("get vintf_ext")
        .expect("vintf_ext some");
    let ret_inner = ret_ext
        .ext
        .get_parcelable::<VintfParcelable>()
        .expect("get inner")
        .expect("inner some");
    assert_eq!(ret_inner.a, 5);
}
...
fn test_repeat_extendable_parcelable_vintf() {
    ...
    ep.ext
        .set_parcelable(Arc::clone(&vintf_ext))
        .expect("error setting parcelable");
...
/// Service name the V1 IFooInterface is registered under by the test service.
/// Must match `FOO_V1_SERVICE_NAME` in `test_service.rs`.
...
fn test_calling_v2_api_triggers_error() {
    ...
    let service: rsbinder::Strong<dyn IFooInterface::IFooInterface> =
        hub::get_interface(FOO_V1_SERVICE_NAME).expect("did not get binder service");

    let ret = service.newApi();
    assert_eq!(
        ret.unwrap_err().transaction_error(),
        StatusCode::UnknownTransaction
    );
}

// ---- trunk-stable cross-version tests (plans/5-aosp-test-porting.md §5) ----
//
// The CLIENT uses the V2 ("current"/notfrozen) ITrunkStableTest; the test
// SERVICE is built from the frozen V1. `getInterfaceVersion()` returns the
// remote (V1) version == 1, so the `version == 1` branches fire — exercising
...
use trunk_v2_gen::android::aidl::test::trunk::ITrunkStableTest::{
    self as TrunkV2, BpTrunkStableTest, IMyCallback, ITrunkStableTest as ITrunkStableTestV2,
    MyEnum::MyEnum as TMyEnum, MyOtherParcelable::MyOtherParcelable as TMyOtherParcelable,
...
fn get_trunk_service() -> rsbinder::Strong<dyn ITrunkStableTestV2> {
    init_test();
    hub::get_interface(<BpTrunkStableTest as ITrunkStableTestV2>::descriptor())
...
fn test_trunk_stable_parcelable() {
    ...
    let p1 = TMyParcelable {
        a: 12,
        b: 13,
        c: 14,
    };
    let p2 = service.repeatParcelable(&p1).expect("repeatParcelable");
    assert_eq!(p1.a, p2.a);
    assert_eq!(p1.b, p2.b);
    if version == 1 {
        // The V1 service neither reads nor echoes `c`; the V2 client leaves it
        // at default via the `has_more_data()` guard.
        assert_ne!(p1.c, p2.c);
        assert_eq!(0, p2.c);
    } else {
        assert_eq!(p1.c, p2.c);
...
fn test_trunk_stable_enum() {
    ...
    // through a V1 service that has no `THREE` enumerator.
    let e1 = TMyEnum::THREE;
    let result = service.repeatEnum(e1).expect("repeatEnum");
    assert_eq!(result, e1);
}
...
fn test_trunk_stable_union() {
    ...
    // `C` is V2-only; a V1 service cannot deserialize that union tag.
    let u1 = TMyUnion::C(14);
...
fn test_trunk_stable_unimplemented() {
    ...
    let p1 = TMyOtherParcelable { a: 12, b: 13 };
    let result = service.repeatOtherParcelable(&p1);
    if version == 1 {
        // V1 has no `repeatOtherParcelable` transaction code.
        assert!(result.is_err());
    } else {
        assert!(result.is_ok());
    }
...
fn test_trunk_stable_hash() {
    ...
    if version == 1 {
        // Remote (V1 service) hash. The client is built at V2, so — unlike
        // AOSP's same-version run — the local consts differ from the remote
        // (this is the cross-version setup; see §5).
        assert_eq!(
            hash.as_ref().map(String::as_str),
...
struct MyCallback {
    repeat_parcelable_called: Arc<Mutex<bool>>,
    repeat_enum_called: Arc<Mutex<bool>>,
    repeat_union_called: Arc<Mutex<bool>>,
    repeat_other_parcelable_called: Arc<Mutex<bool>>,
}
...
fn test_trunk_stable_callback() {
    ...
    let cb = IMyCallback::BnMyCallback::new_binder(MyCallback {
