...
pub enum AidlError {
    ...
    ///
    /// `source` carries the underlying `tera::Error`: its `Display` prints
    /// only the top-level kind ("Failed to render 'interface'"), so the
    /// actionable detail lives in the cause chain.
