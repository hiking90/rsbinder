...
#   * cargo-ndk installed; the rustup target for the device's ABI added.
#
# Usage:
#   ./run_calling_identity_interop.sh [-s emulator-5556] [-t <abi>]
#
...
adb -s "$DEVICE" push \
    "$REPO_ROOT/target/$TRIPLE/release/calling_identity_interop_service" \
    "$SERVICE_BIN" >/dev/null
adb -s "$DEVICE" push \
    "$REPO_ROOT/target/$TRIPLE/release/calling_identity_interop_client" \
    "$CLIENT_BIN" >/dev/null
